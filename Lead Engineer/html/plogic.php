...
function project_details(
    ...
    $sql = "UPDATE lead SET profileimg='$_filenamesprofileimg' where id=$userid";
...
if (isset($_POST['update'])) {
    ...
    if (count($errors) === 0) {
        ...
        //old register start
        /* $query = "INSERT INTO lead SET username=?, email=?, password=?";
...
if (isset($_POST['login-btn'])) {
    ...
    if (count($errors) === 0) {
        $query = "SELECT * FROM lead WHERE username=? OR email=? LIMIT 1";
        $stmt = $conn->prepare($query);
...
if (isset($_POST['login-submit'])) {
    ...
    $sql = "SELECT id,username,password,role,active,disabled,createdat FROM lead WHERE username=? AND password=?";
    ...
    if ($result->num_rows == 1 && $_SESSION['role'] == "client" && $_SESSION['disabled'] == 0 && $_SESSION['active'] == 1) {
        ...
        $msg[] = "Cconn the conn sent your email account to activate your account";
